...
6. Regress the assets by the chosen factor model in the testing sample in a time-series. For instance, when using a Fama French model, the following regression is done for each asset, where $\tilde{r}_i$ is a vector of excess of returns of test asset, $i$, $r_MKT$ is a vector of returns of the market factor (which represents the return of the market minus the return of the risk-free rate), $r_{\text{HML}}$ is a vector of returns of the HML factor (High book-to-market ratio stocks minus low book-to-market ratio stocks), and $r_{\text{SMB}}$ is a vector of returns of the SMB factor (small caps minus big caps):
